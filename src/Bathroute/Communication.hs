  ...
  case decodeStrict msg of
    ...
    Just sc → case decodeMessage sk sc of
      ...
      Right (pk, r) → yield =<< case r of
        OnlineStatus x → return $ Failed "OnlineStatus not implemented"
...
insertAlias pk (DesiredAlias s) = S.runSqlite ":memory:" $ do
  als ← E.select . E.from $ \al → do
    E.where_ (al E.^. AliasName E.==. E.val s)
  case als of
    [] → insert (Alias s . LB.toStrict $ B.encode pk) >> return OK
